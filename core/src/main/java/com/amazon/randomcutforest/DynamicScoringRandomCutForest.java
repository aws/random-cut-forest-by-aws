...
/**
 * DynamicRandomCutForest extends {@link RandomCutForest} to add methods for computing scalar scores using scoring
 * functions which can be passed in as lambdas.
 */
public class DynamicScoringRandomCutForest extends RandomCutForest {
    ...
    /**
     * Create a new DynamicScoringRandomCutForest with optional arguments set to default values.
     *
     ...
     * @param seen                    the function that applies if input is equal to a previously seen sample in a leaf
     * @param unseen                  if the input does not have a match in the leaves
     * @param damp                    damping function based on the duplicity of the previously seen samples
     * @return anomaly score
     */
    public double getDynamicScore(double[] point, int ignoreLeafMassThreshold,
        ...
        Function<RandomCutTree, Visitor<Double>> visitorFactory = tree ->
                new DynamicScoreVisitor(point, tree.getRoot().getMass(), ignoreLeafMassThreshold,
                        seen, unseen, damp);
     ...
     * probabilities (vector over the dimensions); and produces a score af-if the tree
     * were built using that function (when in reality the tree is an RCF). Changing the
     * defaultRCFgVec function to some other function f() will provide a mechanism of dynamic
     * scoring for trees that are built using f() which is the purpose of
     * TransductiveScalarScore visitor. Note that the answer is an MCMC simulation and is not normalized
     * (because the scoring functions are flexible and unknown) and over a small number of trees
     * the errors can be large specially if vecSep is very far from defaultRCFgVec
     *
    ...
    public double getDynamicSimulatedScore(double[] point,
                                           BiFunction<Double, Double, Double> seen, BiFunction<Double, Double, Double> unseen,
                                           BiFunction<Double, Double, Double> damp,
        ...
        Function<RandomCutTree, Visitor<Double>> visitorFactory = tree ->
                new SimulatedTransductiveScalarScoreVisitor(point, tree.getRoot().getMass(),
                        seen, unseen, damp, CommonUtils::defaultRCFgVecFunction, vecSep);
        BinaryOperator<Double> accumulator = Double::sum;
    ...
    /**
     * Score a point using the given scoring functions. This method will short-circuit before visiting all trees
     * if the scores that are returned from a subset of trees appears to be converging to a given value. See
     * {@link OneSidedConvergingDoubleAccumulator} for more about convergence.
     *
     * @param point                   input point
     * @param precision               controls early convergence
     * @param highIsCritical          this is true for the default scoring function. If the user wishes to use a
     *                                different scoring function where anomaly scores are low values (for example, height in tree)
     *                                then this should be set to false.
     * @param ignoreLeafMassThreshold said threshold
     * @param seen                    scoring function when the input matches some tuple in the leaves
     * @param unseen                  scoring function when the input is not found
     * @param damp                    dampening function for duplicates which are same as input (applies with seen)
    ...
    /**
     * Same as above, but for dynamic scoring. See the params of getDynamicScoreParallel
     *
     * @param point                   point to be scored
     * @param ignoreLeafMassThreshold said threshold
     * @param seen                    score function for seen points
     * @param unseen                  score function for unseen points
     * @param newDamp                 dampening function for duplicates in the seen function
    ...
    public DiVector getDynamicAttribution(double[] point, int ignoreLeafMassThreshold,
        ...
        Function<RandomCutTree, Visitor<DiVector>> visitorFactory = tree ->
                new DynamicAttributionVisitor(point, tree.getRoot().getMass(),
    ...
    /**
     * Atrribution for dynamic sequential scoring; getL1Norm() should agree with getDynamicScoringSequential
     *
     * @param point                   input
     * @param precision               parameter to stop early stopping
     * @param highIsCritical          are high values anomalous (otherwise low values are anomalous)
     * @param ignoreLeafEquals        should we ignore leaves with mass equal/below threshold
     * @param ignoreLeafMassThreshold said threshold
     * @param seen                    function for scoring points that have been seen before
    ...
    public DiVector getApproximateDynamicAttribution(double[] point, double precision, boolean highIsCritical,
            boolean ignoreLeafEquals, int ignoreLeafMassThreshold,
            BiFunction<Double, Double, Double> seen, BiFunction<Double, Double, Double> unseen,
            BiFunction<Double, Double, Double> newDamp) {
        ...
        Function<RandomCutTree, Visitor<DiVector>> visitorFactory = tree ->
                new DynamicAttributionVisitor(point, tree.getRoot().getMass(), ignoreLeafMassThreshold,
                        seen, unseen, newDamp);

        ConvergingAccumulator<DiVector> accumulator = new OneSidedConvergingDiVectorAccumulator(dimensions, highIsCritical,
