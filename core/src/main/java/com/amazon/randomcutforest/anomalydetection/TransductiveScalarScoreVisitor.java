...
public class TransductiveScalarScoreVisitor extends DynamicScoreVisitor {
    ...
    /* the goal of this visitor is to allow tranductive inference; where during scoring
     * we make adjustments so that it appears (to the best of simulation ability) that
     * the tree was built using the knowledge of the point being scored
     ...
     * @param vecSep                  A function that provides the probabilities of choosing different
     *                                dimensions given a BoundingBox when the tree was built. This must be
     *                                the same as the probabilies of Transductive inference during scoring.
     *                                For extenstions where these are different,
     *                                see SimulatedTransductiveScalarScoreVisitor
     *
     *  Note that scores are not normalized because the function ranges are unknown
     *  as is the case with DynamicScoreVisitor
     */
    ...
    // same probabilities as are used in scoring. In the application herein vecSepBuild
    // is the same as vecSepScore as in the accept(node) above; however the function is
    // written in the more general form so that it can be used for the Simulated
    // version as well without any changes.

    protected double getWeight(int dim, Function<BoundingBox, double[]> vecSepBuild,
        ...
        return (boundingBox.getRange(dim) / largeBox.getRange(dim))
                * (sumSmall / sumLarge) * (vecLarge[dim] / vecSmall[dim]);
