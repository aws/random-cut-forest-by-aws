...
/**
 * Summarized representation of the stored points provide a convenient view into the "current state" of the stream
 * seen/sampled by an RCF. However since RCFs provide a generic sketch for multple different scenrios
 * https://opensearch.org/blog/odfe-updates/2019/11/random-cut-forests/
 * the summarization can be used repeatedly to provide a dynamic clustering a numeric  data stream as shown in the
 * example below.
 *
 * The summarization is based on a well-scattered multi-centroid representation as in CURE
 * https://en.wikipedia.org/wiki/CURE_algorithm
 * and distance based clustering as in https://en.wikipedia.org/wiki/Data_stream_clustering
 *
 * The example corresponds to a wheel like arrangement -- where numberOfBlades determine the number of spokes.
 * For many settings of the parameter the spokes are closer to each other near the center than the extremity at
 * the rim. Thus a centroidal representation cannot conceptually capture each spoke as a cluster, and multi-centroid
 * approach is necessary. Note that the input to the summarization is not the same as the numberOfBladed; the maxAllowed
 * number corresponds to the maximum number of clusters which can be much larger. In a clustering application, the number
 * of clusters are typically not known apriori.
 *
 * The pointset is generated once and are input to RCF with rotations. As the "blades are running", the output clusters
 * can be colored and we can visualize the clusters produced. For the parameters below, simplistic plotting functions
 * such as gnuplot using
 * do for [i = 0:359] {
 *     plot [-15:15][-15:15] "sum" index i u 1:2:3:4 w circles fill solid noborder fc palette z t ""
 *     }
 * would show the rotating clusters where the representatives corresponding to the same cluster has the same color.
 * We note that the visualizations is neither polished nor complete, since the goal is to highlight the functionality
 * of summarization in RCFs.
 */
public class DynamicSummarization implements Example {
    ...
    public void run() throws Exception {
        ...
        for (int degree = 0; degree < 360; degree += 1) {
            ...
            // if the maxAllowed is increased, decrease the shrikage ratio to get counterbalance the test for
            // sumary.size() == numberOfFans
            // for example,
            // List<ICluster<float[]>> summary = newForest.summarize(5*numberOfFans,0.1,5,0.5,oldSummary);

            List<ICluster<float[]>> summary = newForest.summarize(2*numberOfBlades,shrinkage,5,DEFAULT_SEPARATION_RATIO_FOR_MERGE,oldSummary);
            sum += summary.size();
        ...
        System.out.println("Exact detection :" + ((float) Math.round(count/3.6)*0.01) + " fraction, average number of clusters " + ((float) Math.round(sum/3.6)*0.01));
        file.close();
    }
