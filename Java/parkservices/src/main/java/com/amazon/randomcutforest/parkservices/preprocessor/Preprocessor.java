...
public class Preprocessor implements IPreprocessor<AnomalyDescriptor> {
    ...
    protected long inverseMapTimeValue(double gap, long timestamp) {
        ...
        if (normalizeTime) {
            ...
            return (long) Math.floor(gap / factor + timestamp);
        }
    }
    ...
    void invertTimeForecast(RangeVector ranges, long timeStamp) {
        ...
        int baseDimension = inputLength + 1;
        int horizon = ranges.values.length / baseDimension;
        for (int i = 0; i < horizon; i++) {
            int index = i * baseDimension + inputLength;
            ranges.values[index] = (float) inverseMapTimeValue(ranges.values[index], localTimestamp);
            ranges.lower[index] = (float) inverseMapTimeValue(ranges.lower[index], localTimestamp);
            ranges.upper[index] = (float) inverseMapTimeValue(ranges.lower[index], localTimestamp);
            localTimestamp = (long) ranges.values[index];
        }
    }
     ...
     */
    public void invertForecastRange(RangeVector ranges, IRCFComputeDescriptor lastAnomalyDescriptor) {
        int baseDimension = inputLength
                + ((lastAnomalyDescriptor.getForestMode() == ForestMode.TIME_AUGMENTED) ? 1 : 0);
