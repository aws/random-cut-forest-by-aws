...
public class ExtendedRandomCutForest {
    ...
    // the number of scores we should see before vending judgements of
    // anomaly/anomaly
    // more is better, the setting below is an optimistic setting
    private int minimumScores = 10;

    // the score below which we will ignore anomalies; there can be examples where
    // the setting of 1.0
    // is large -- but the setting below (and increasing it) should reduce the
    ...
    public ExtendedRandomCutForest(RandomCutForest.Builder<?> builder, double anomalyRate) {
        ...
        correctorThresholder = new CorrectorThresholder(anomalyRate, baseDimensions, shingleSize, true, lowerThreshold,
                minimumScores, false);
    }

    public ExtendedRandomCutForest(RandomCutForest forest, CorrectorThresholder thresholder, int count,
            int numberOfAttributors, double[] lastAnomalyPoint, int lastAnomalyTimeStamp) {
        checkArgument(!forest.isInternalShinglingEnabled(), "internal shingling not suported");
        this.forest = forest;
        baseDimensions = forest.getDimensions() / forest.getShingleSize();
        shingleSize = forest.getShingleSize();
        ...
        this.lastAnomalyPoint = (lastAnomalyPoint == null) ? null
                : Arrays.copyOf(lastAnomalyPoint, lastAnomalyPoint.length);
    }
    ...
    public AnomalyDescriptor process(double[] point) {
        ...
        if (result.getRcfScore() <= 0
                || correctorThresholder.process(result.getRcfScore(), count) != BasicThresholder.MORE_INFORMATION) {
        ...
        /**
         * we consider what the most recent values should have been, reflected in
         * newPoint; and then pass the score, score of newPoint, attribution and
         * attribution of newPoint to the thresholder. The idea is that "if the most
         * likely least anomalous score is high" (reflected in
         * forest.getAnomalyScore(newPoint) then the most recent observations are not an
         * anomaly. If the still are considered an anomaly, then we look at the most
         * egregious subobservations in the shingle, given by maxContribution() and
         * predict those values -- note that this may correspond to anomalies being
         * detecting late; and deciding on the values when we detect anomalies (based on
         * what we know now, as opposed to pure forecasting)
         *
         * The parameter CONFIG_NUMBER_OF_ATTRIBUTORS determines the maximum number of
         * different attributors we could consider; note that larger number of
         * contributors are difficult to visualize/control
         */
        ...
        if (reasonableForecast && lastAnomalyPoint != null && count - lastAnomalyTimeStamp < shingleSize) {
            ...
            if (!correctorThresholder.isPotentialAnomaly(correctedScore)) {
                ...
                // score
                correctorThresholder.process(result.getRcfScore(), correctedScore, result.getAttribution(), null,
                        count);
                result.setAnomalyGrade(0);
                ++count;
                forest.update(point);
        ...
        int signal = correctorThresholder.process(result.getRcfScore(), newScore, result.getAttribution(),
                newAttribution, count);

        if (signal > 0) {
            result.setAnomalyGrade(0.01 * signal);
            result.setRelativeIndex(index);
            result.setStartOfAnomaly(!inAnomaly);
            result.setExpectedValuesPresent(
                    (count > MINIMUM_OBSERVATIONS_FOR_EXPECTED) && (shingleSize * baseDimensions >= 4));
            if (result.getRelativeIndex() < 0 && result.isStartOfAnomaly()) {
                // anomaly in the past and detected late
                startPosition = result.getAttribution().getDimensions()
                        + (result.getRelativeIndex() - 1) * baseDimensions;
                if (result.isExpectedValuesPresent()) {
                    int[] missingIndices = largestFeatures(result.getAttribution(), startPosition, baseDimensions,
                            numberOfAttributors);
