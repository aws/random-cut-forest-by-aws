...
public class CorrectorThresholder {
    ...
    public CorrectorThresholder(RandomCutForest forest, double anomalyRate) {
        this.forest = forest;
        thresholder = new BasicThresholder(anomalyRate);
    }

    protected void setForest(RandomCutForest forest){
        this.forest = forest;
    }

    public CorrectorThresholder(BasicThresholder thresholder, CorrectorThresholderState state){
    ...
    public AnomalyDescriptor process(double[] point) {
        ...
        if (reasonableForecast && lastAnomalyPoint != null && gap < shingleSize) {
            double[] correctedPoint = Arrays.copyOf(point, point.length);
            for (int i = 0; i < point.length - gap * baseDimensions; i++) {
                correctedPoint[i] = lastAnomalyPoint[i + gap * baseDimensions];
            }
