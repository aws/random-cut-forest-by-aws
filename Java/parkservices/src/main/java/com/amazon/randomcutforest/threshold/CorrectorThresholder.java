...
public class CorrectorThresholder extends BasicThresholder {
    ...
    // a conservative upper bound of zFactor, when we are seeing a continuous run of
    // anomalies
    ...
    // a statistical measure of score difference (ignoring the anomaly -> not an
    // anomaly transition)
    ...
    public CorrectorThresholder(double discount, int baseDimension, int shingleSize, boolean attributionEnabled,
            double absoluteThreshold, int minimumScores, boolean ignoreSimilar) {
    ...
    public CorrectorThresholder(CorrectorThresholderState state, Deviation simpleDeviation, Deviation scoreDiff) {
        ...
        this.lastAnomalyAttribution = (state.getLastAnomalyAttribution() != null)
    ...
    public int process(double newScore, double idealScore, DiVector attribution, DiVector idealAttribution,
            int timeStamp) {
    ...
    protected boolean trigger(DiVector candidate, int timeStamp, DiVector ideal) {
        ...
        if (difference < dimensions) {
            if (ideal == null) {
                ...
                return (remainder * dimensions / difference - simpleDeviation.getMean() > triggerFactor
                        * simpleDeviation.getDeviation());
            } else {
                ...
                return (differentialRemainder > ignoreSimilarFactor * lastAnomalyScore)
                        && (differentialRemainder * dimensions / difference - simpleDeviation.getMean() > triggerFactor
                                * simpleDeviation.getDeviation());
            }
        } else {
            if (!ignoreSimilar) {
    ...
    public int getGrade(double score) {
        ...
        double absolutePart = (1 - interpolate) * (normalizedScore - lowerThreshold)
