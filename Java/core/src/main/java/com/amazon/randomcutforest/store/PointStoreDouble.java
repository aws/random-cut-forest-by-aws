...
public class PointStoreDouble implements IPointStore<double[]> {
    ...
    public static int INFEASIBLE_LOCN = (int) -2;
    /**
    ...
    public int decrementRefCount(int index) {
        checkArgument(index >= 0 && index < locationList.length, " index not supported by store");
    ...
    int takeIndex() {
        if (indexManager.isEmpty()) {
            if (indexManager.getCapacity() < capacity) {
                ...
                int oldlocationLength = locationList.length;
                locationList = Arrays.copyOf(locationList, newCapacity);
                for (int i = oldlocationLength; i < newCapacity; i++) {
            ...
            for (int i = blockStart; i < blockEnd; i++) {
                ...
                if (jStatic < jEnd) {
                    ...
                    if (i == locn) {
                        ...
                        locationList[newIdx] = freshStart;
                        jStatic += 1;
                    }
                }
                freshStart += 1;
    ...
    public PointStoreDouble(int dimensions, int capacity) {
        this(PointStoreDouble.builder().capacity(capacity).dimensions(dimensions).shingleSize(1).initialSize(capacity));
    }
