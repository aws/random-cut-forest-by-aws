...
/**
 * A utility class for creating a {@link RandomCutForestState} instance from a {@link
 * RandomCutForest} instance and vice versa.
 */
@Getter
@Setter
public class RandomCutForestMapper
    ...
    /**
     * A flag indicating whether the structure of the trees in the forest should be included in the
     * state object. If true, then data describing the bounding boxes and cuts defining each tree
     * will be written to the {@link RandomCutForestState} object produced by the mapper. Tree state
     * is not saved by default.
     */
    private boolean saveTreeStateEnabled = false;

    /**
     * A flag indicating whether the point store should be included in the {@link
     * RandomCutForestState} object produced by the mapper. This is saved by default for compact
     * trees
     */
    private boolean saveCoordinatorStateEnabled = true;

    /**
     * A flag indicating whether the samplers should be included in the {@link RandomCutForestState}
     * object produced by the mapper. This is saved by default for all trees.
     */
    private boolean saveSamplerStateEnabled = true;

    /**
     * A flag indicating whether the executor context should be included in the {@link
     * RandomCutForestState} object produced by the mapper. Executor context is not saved by defalt.
     */
    ...
    /**
     * if true would require that the samplers populate the trees before the trees can be used gain.
     * That woukld correspond to extra time, at the benefit of a smaller serialization.
     */
    private boolean partialTreeStateEnabled = false;

    /**
     * Create a {@link RandomCutForestState} object representing the state of the given forest. If
     * the forest is compact and the {@code saveTreeState} flag is set to true, then structure of
     * the trees in the forest will be included in the state object. If the flag is set to false,
     * then the state object will only contain the sampler data for each tree. If the {@code
     * saveExecutorContext} is true, then the executor context will be included in the state object.
     *
     * @param forest A Random Cut Forest whose state we want to capture.
     * @return a {@link RandomCutForestState} object representing the state of the given forest.
     * @throws IllegalArgumentException if the {@code saveTreeState} flag is true and the forest is
     *     not compact.
     */
    @Override
    public RandomCutForestState toState(RandomCutForest forest) {
        ...
        if (forest.isCompact()) {
            if (saveCoordinatorStateEnabled) {
                PointStoreCoordinator<?> pointStoreCoordinator =
                        (PointStoreCoordinator<?>) forest.getUpdateCoordinator();
                ...
                if (forest.getPrecision() == Precision.FLOAT_32) {
                    ...
                    pointStoreState =
                            mapper.toState((PointStoreFloat) pointStoreCoordinator.getStore());
                } else {
                    PointStoreDoubleMapper mapper = new PointStoreDoubleMapper();
                    mapper.setCompressionEnabled(compressionEnabled);
                    pointStoreState =
                            mapper.toState((PointStoreDouble) pointStoreCoordinator.getStore());
                }
            ...
            for (IComponentModel<?, ?> component : forest.getComponents()) {
                SamplerPlusTree<Integer, ?> samplerPlusTree =
                        (SamplerPlusTree<Integer, ?>) component;
            ...
            if (trees != null) {
                if (forest.getPrecision() == Precision.FLOAT_32) {
                    CompactRandomCutTreeFloatMapper treeMapper =
                            new CompactRandomCutTreeFloatMapper();
                    ...
                    List<CompactRandomCutTreeState> treeStates =
                            trees.stream()
                                    .map(t -> treeMapper.toState((CompactRandomCutTreeFloat) t))
                                    .collect(Collectors.toList());
                    state.setCompactRandomCutTreeStates(treeStates);
                } else {
                    CompactRandomCutTreeDoubleMapper treeMapper =
                            new CompactRandomCutTreeDoubleMapper();
                    treeMapper.setCompress(compressionEnabled);
                    treeMapper.setPartialTreeStateEnabled(
                            partialTreeStateEnabled || forest.isStoreSequenceIndexesEnabled());
                    List<CompactRandomCutTreeState> treeStates =
                            trees.stream()
                                    .map(t -> treeMapper.toState((CompactRandomCutTreeDouble) t))
                                    .collect(Collectors.toList());
                    state.setCompactRandomCutTreeStates(treeStates);
                }
            }
        } else {
            ArraySamplersToCompactStateConverter converter =
                    new ArraySamplersToCompactStateConverter(
                            forest.isStoreSequenceIndexesEnabled(),
            ...
            for (IComponentModel<?, ?> model : forest.getComponents()) {
                ...
                SimpleStreamSampler<double[]> sampler =
                        (SimpleStreamSampler<double[]>) samplerPlusTree.getSampler();
    ...
    /**
     * Create a {@link RandomCutForest} instance from a {@link RandomCutForestState}. If the state
     * contains tree states, then trees will be constructed from the tree state objects. Otherwise,
     * empty trees are created and populated from the sampler data. The resulting forest should be
     * equal in distribution to the forest that the state object was created from.
     *
     * @param state A Random Cut Forest state object.
     * @param executorContext An executor context that will be used to initialize new executors in
     *     the Random Cut Forest. If this argument is null, then the mapper will look for an
     *     executor context in the state object.
     * @param seed A random seed.
     * @return A Random Cut Forest corresponding to the state object.
     * @throws NullPointerException if both the {@code executorContext} method argument and the
     *     executor context field in the state object are null.
     */
    public RandomCutForest toModel(
            RandomCutForestState state, ExecutorContext executorContext, long seed) {
        ...
        RandomCutForest.Builder<?> builder =
                RandomCutForest.builder()
                        .numberOfTrees(state.getNumberOfTrees())
                        .dimensions(state.getDimensions())
                        .lambda(state.getTimeDecay())
                        .sampleSize(state.getSampleSize())
                        .centerOfMassEnabled(state.isCenterOfMassEnabled())
                        .outputAfter(state.getOutputAfter())
                        .parallelExecutionEnabled(ec.isParallelExecutionEnabled())
                        .threadPoolSize(ec.getThreadPoolSize())
        ...
        for (int i = 0; i < state.getNumberOfTrees(); i++) {
            ...
            SimpleStreamSampler<double[]> sampler =
                    new SimpleStreamSampler<>(
                            state.getSampleSize(), state.getTimeDecay(), rng.nextLong());
            ...
            for (Weighted<Integer> sample : compactData.getWeightedSample()) {
                ...
                sampler.addSample(
                        new Weighted<>(point, sample.getWeight(), sample.getSequenceIndex()));
    ...
    /**
     * Create a {@link RandomCutForest} instance from a {@link RandomCutForestState} using the
     * executor context in the state object. See {@link #toModel(RandomCutForestState,
     * ExecutorContext, long)}.
     *
     * @param state A Random Cut Forest state object.
     * @param seed A random seed.
     * @return A Random Cut Forest corresponding to the state object.
     * @throws NullPointerException if the executor context field in the state object are null.
     */
    ...
    public RandomCutForest singlePrecisionForest(
            RandomCutForest.Builder<?> builder,
            RandomCutForestState state,
            IPointStore<float[]> extPointStore,
            List<ITree<Integer, float[]>> extTrees,
            List<IStreamSampler<Integer>> extSamplers) {

        checkArgument(builder != null, "builder cannot be null");
        checkArgument(
                extTrees == null || extTrees.size() == state.getNumberOfTrees(),
                "incorrect number of trees");
        checkArgument(
                extSamplers == null || extSamplers.size() == state.getNumberOfTrees(),
                "incorrect number of samplers");
        checkArgument(extSamplers != null | state.isSaveSamplerStateEnabled(), " need samplers ");
        checkArgument(
                extPointStore != null || state.isSaveCoordinatorStateEnabled(),
                " need coordinator state ");
        ...
        List<CompactRandomCutTreeState> treeStates =
                state.isSaveTreeStateEnabled() ? state.getCompactRandomCutTreeStates() : null;
        CompactSamplerMapper samplerMapper = new CompactSamplerMapper();
        List<CompactSamplerState> samplerStates =
                state.isSaveSamplerStateEnabled() ? state.getCompactSamplerStates() : null;
        for (int i = 0; i < state.getNumberOfTrees(); i++) {
            ...
            if (extTrees != null) {
                ...
                if (treeStates.get(i).isPartialTreeState()) {
                    sampler.getSample()
                            .forEach(s -> tree.addPoint(s.getValue(), s.getSequenceIndex()));
                }
            } else {
                tree =
                        new CompactRandomCutTreeFloat.Builder()
                                .maxSize(state.getSampleSize())
                                .randomSeed(rng.nextLong())
    ...
    public RandomCutForest doublePrecisionForest(
            RandomCutForest.Builder<?> builder,
            RandomCutForestState state,
            IPointStore<double[]> extPointStore,
            List<ITree<Integer, double[]>> extTrees,
            List<IStreamSampler<Integer>> extSamplers) {
