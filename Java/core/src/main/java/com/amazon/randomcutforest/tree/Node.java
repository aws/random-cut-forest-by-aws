...
/**
 * A Node in a {@link RandomCutTree}. All nodes contain references to the parent and children nodes
 * (which may be null, in the case of the root node or a leaf node). All nodes also contain a {@link
 * BoundingBox}, which is the smallest BoundingBox that contains all points that are descendents of
 * the given node. Leaf nodes additionally contain a point value.
 */
public class Node implements INode<Node> {
    ...
    /**
     * If this is a non-leaf node and the center of mass computation has been enabled in
     * RandomCutTree, this array will store the sum of all descendent points.
     */
    ...
    /**
     * For a non-leaf node this is a {@link Cut} that divides this node's bounding box into two
     * sections. This cut also determines the canonical root-to-leaf traversal path for a given
     * point.
     */
    private Cut cut;
    ...
    private int mass;
    /**
     * If this is a leaf node and sequence indexes have been enabled in the RandomCutTree, this set
     * stores the indexes corresponding to times when the given leaf point was added to the tree.
     */
     ...
     * @param enableCenterOfMass A flag indicating whether center of mass computations will be
     *     applied tot his node.
     */
    public Node(
    ...
    /**
     * Return this node's bounding box. This will be the smallest bounding box that contains all
     * descendent points for this node.
     *
    ...
    /**
     * For a leaf node, test whether this given point is equal to the leaf point. This test equality
     * as double values (that is, the method will not return true of the given point and the leaf
     * point are merely close). For a non-leaf node this method will always return false.
     *
     * @param point A point that we are comparing to this node's leaf point.
     * @return true if this is a leaf node and the point is equal to the leaf point, false
     *     otherwise.
     */
    ...
    /**
     * Return the mass of this node. For a leaf node, this will be equal to the number of times that
     * the leaf point has been added to the tree. For a non-leaf node, this will be equal to the
     * total mass of all descendent points.
     *
    ...
    /**
     * Return the value of this node's point sum. For leaf node, this will be the leaf mass times
     * the leaf point value. For a non-leaf node with center of mass computation enabled, this will
     * be the sum of all descendent points. For a non-leaf node with center of mass computation
     * disabled, this will be an array of 0s.
     *
    ...
    /**
     * If the option to compute center of mass is enabled in the RandomCutTree that this Node
     * belongs to, this method will return the center of mass of all the points contained in this
     * node. If the option is not enabled, it returns an array of all 0s.
     *
    ...
    /**
     * Return an unmodifiable set of sequence indexes. These are ordinals which indicate the times
     * when this point was added to the tree. If this is an interior node or if storing sequence
     * indexes is disable, the set will be empty.
     *
    ...
    /**
     * The following function returns the bounding box corresponding to the child that is not the
     * path from the node to leaf traversed by a point. If such a box cannot be defined then the
    ...
    public BoundingBox getSiblingBoundingBox(double[] point) {
        Node siblingOfPath =
                Node.isLeftOf(point, this) ? this.getRightChild() : this.getLeftChild();
    ...
    BoundingBox constructBoxInPlace(BoundingBox currentBox) {
        if (isLeaf()) {
            ...
            return getRightChild()
                    .constructBoxInPlace(getLeftChild().constructBoxInPlace(currentBox));
        }
    }
