...
/**
 * This accumulator checks to see if a result is converging by testing the sample mean and standard
 * deviation of a scalar value computed from the result. As the name implies, the accumulator
 * performs a one-sided check, comparing the new value the current sample mean and updating its
 * converged status only if the difference is positive (or negative, if highIsCritical is set to
 * false. This accumulator is intended to be used with values where we care more about outliers in
 * one direction. For example, if our statistic is anomaly score, we are normally more concerned
 * with high anomaly scores than low ones.
 *
...
public abstract class OneSidedStDevAccumulator<R> implements ConvergingAccumulator<R> {
    ...
    /**
     * When testing for convergence, we use ALPHA times the sample standard deviation to define our
     * interval.
     */
    private static final double ALPHA = 0.5;
    /**
     * The minimum number of values that have to be accepted by this accumulator before we start
    ...
    /**
     * Set to 'true' if we care more about high values of the converging scalar than low values. Set
     * to 'false' if the opposite is true.
     */
    private final boolean highIsCritical;
    /**
     * This value is +1 if highIsCritical is 'true', and -1 if highIsCritical is fault. It is used
     * in the converegence test.
     */
    ...
    /**
     * The current sum of squares of the converging scalar value. Used to compute the sample
     * standard deviation.
     */
     ...
     *
     * @param highIsCritical Set to 'true' if we care more about high values of the converging
     *     scalar than low values. Set to 'false' if the opposite is true.
     * @param precision The number of witnesses required before declaring convergence will be at
     *     least 1.0 / precision.
     * @param minValuesAccepted The user-specified minimum number of values visited before returning
     *     a result. Note that {@link #isConverged()} may return true before accepting this number
     *     of results if the
     * @param maxValuesAccepted The maximum number of values that will be accepted by this
     *     accumulator.
     */
    public OneSidedStDevAccumulator(
            boolean highIsCritical,
            double precision,
            int minValuesAccepted,
            int maxValuesAccepted) {

        this.highIsCritical = highIsCritical;
        this.convergenceThreshold =
                precision < 1.0 / maxValuesAccepted ? maxValuesAccepted : (int) (1.0 / precision);
