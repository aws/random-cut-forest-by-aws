...
/**
 * DynamicRandomCutForest extends {@link RandomCutForest} to add methods for computing scalar scores
 * using scoring functions which can be passed in as lambdas.
 */
public class DynamicScoringRandomCutForest extends RandomCutForest {
    ...
    public double getDynamicScore(
            ...
            BiFunction<Double, Double, Double> unseen,
            BiFunction<Double, Double, Double> damp) {

        checkArgument(
                ignoreLeafMassThreshold >= 0,
                "ignoreLeafMassThreshold should be greater than or equal to 0");
        ...
        VisitorFactory<Double> visitorFactory =
                tree ->
                        new DynamicScoreVisitor(
                                point, tree.getMass(), ignoreLeafMassThreshold, seen, unseen, damp);
    ...
    /**
     * Similar to above but now the scoring takes in a function of Bounding Box to probabilities
     * (vector over the dimensions); and produces a score af-if the tree were built using that
     * function (when in reality the tree is an RCF). Changing the defaultRCFgVec function to some
     * other function f() will provide a mechanism of dynamic scoring for trees that are built using
     * f() which is the purpose of TransductiveScalarScore visitor. Note that the answer is an MCMC
     * simulation and is not normalized (because the scoring functions are flexible and unknown) and
     * over a small number of trees the errors can be large specially if vecSep is very far from
     * defaultRCFgVec
     *
     * <p>Given the large number of possible sources of distortion, ignoreLeafThreshold is not
     * supported.
     *
     ...
     */
    public double getDynamicSimulatedScore(
            double[] point,
            BiFunction<Double, Double, Double> seen,
            BiFunction<Double, Double, Double> unseen,
            BiFunction<Double, Double, Double> damp,
    ...
    /**
     * Score a point using the given scoring functions. This method will short-circuit before
     * visiting all trees if the scores that are returned from a subset of trees appears to be
     * converging to a given value. See {@link OneSidedConvergingDoubleAccumulator} for more about
     * convergence.
     *
     * @param point input point
     * @param precision controls early convergence
     * @param highIsCritical this is true for the default scoring function. If the user wishes to
     *     use a different scoring function where anomaly scores are low values (for example, height
     *     in tree) then this should be set to false.
     * @param ignoreLeafMassThreshold said threshold
     ...
     */
    public double getApproximateDynamicScore(
            double[] point,
            double precision,
     ...
     */
    public DiVector getDynamicAttribution(
            double[] point,
            int ignoreLeafMassThreshold,
            BiFunction<Double, Double, Double> seen,
            BiFunction<Double, Double, Double> unseen,
     ...
     */
    public DiVector getApproximateDynamicAttribution(
            double[] point,
            double precision,
            boolean highIsCritical,
            boolean ignoreLeafEquals,
            int ignoreLeafMassThreshold,
            BiFunction<Double, Double, Double> seen,
            BiFunction<Double, Double, Double> unseen,
            BiFunction<Double, Double, Double> newDamp) {
