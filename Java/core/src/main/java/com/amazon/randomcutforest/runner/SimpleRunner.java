...
/**
 * A simple command-line application that parses command-line arguments, creates a RandomCutForest
 * instance based on those arguments, reads values from STDIN and writes results to STDOUT.
 */
public class SimpleRunner {
     ...
     * @param runnerDescription A description of the runner class. This will be displayed in the
     *     help text.
     * @param algorithmInitializer A factory method to create a new LineTransformer instance from a
     *     RandomCutForest.
     */
    public SimpleRunner(
            String runnerClass,
     ...
     * Create a new SimpleRunner.
     *
     * @param argumentParser A argument parser that will be used by this runner to parse
    ...
    /**
     * Read data from an input stream, apply the desired transformation, and write the result to an
     * output stream.
     *
     ...
     *
     * @param values The array of values that are used to create the header. These values will be
     *     joined together using the user-specified delimiter.
     ...
     *
     * @param values An array of string values taken from the input stream. These values will be
     *     parsed into an array of doubles before being transformed and written to the output
     *     stream.
    ...
    protected void processLine(String[] values, PrintWriter out) {
        if (values.length != pointBuffer.length) {
            throw new IllegalArgumentException(
                    String.format(
                            "Wrong number of values on line %d. Exected %d but found %d.",
    ...
    /**
     * This method is used to write any final output to the output stream after the input stream has
     * beeen fully processed.
     *
