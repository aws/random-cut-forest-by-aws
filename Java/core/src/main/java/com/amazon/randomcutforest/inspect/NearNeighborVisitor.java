...
/**
 * A visitor that returns the leaf node in a traversal if the distance between the leaf point and
 * the query point is less than a given threshold.
 */
public class NearNeighborVisitor implements Visitor<Optional<Neighbor>> {
     ...
     * @param distanceThreshold Leaf points whose distance from the query point is less than this
     *     value are considered near neighbors.
     */
    ...
    /**
     * Create a NearNeighborVisitor which always returns the leaf point in the traversal. The
     * distance threshold is set to positive infinity.
     *
     * @param queryPoint The point whose neighbors we are looking for.
     */
    ...
    /**
     * Near neighbors are identified in the {@link #acceptLeaf} method, hence this method does
     * nothing.
     *
    ...
    /**
     * Check to see whether the Euclidean distance between the leaf point and the query point is
     * less than the distance threshold. If it is, then this visitor will return an {@link
     * java.util.Optional} containing this leaf point (converted to a {@link Neighbor} object).
     * Otherwise, this visitor will return an empty Optional.
     *
    ...
    /**
     * @return an {@link Optional} containing the leaf point (converted to a {@link Neighbor} if the
     *     Euclidean distance between the leaf point and the query point is less than the distance
     *     threshold. Otherwise return an empty Optional.
     */
    @Override
