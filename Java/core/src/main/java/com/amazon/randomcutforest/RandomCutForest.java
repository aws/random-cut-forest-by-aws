...
public class RandomCutForest {
    ...
    /**
     * an L1 clustering primitive that shows the aggregation of the points stored in RCF
     * the clustering uses multi-centroid clustering introduced in CURE
     * https://en.wikipedia.org/wiki/CURE_algorithm
     * However CURE also shrunk the well scattered points by a fraction alpha (there by creating new points); while that
     * concept is used herein, the (multi) summarization algorithm changes the distance metric as opposed to creating new
     * points since continuity of values is not an useful assumption in context of RCFs. The usage of distance metric
     * is similar to the discussion in https://en.wikipedia.org/wiki/Data_stream_clustering
     * See the examples package for an example of dynamic summarization.
     * /
     * @param maxAllowed maximum number of clusters one is willing to see
     * @param shrinkage a parameter that controls between spherical nature (=1) and MST (=0), this corresponds to
     *                  the parameter alpha in the description above
     * @param numberOfRepresentatives number of centroids used to represent a cluster, this is the parameter c
     *                                in the description of CURE
     * @param separationRatio a parameter in [0,1] that controls how zealously should the algorithm reduce the
     *                        number of clusters a default value of 0.8 is a reasonable value for many settings.
     *                        A value close to 0 would tend to merge eveything into a single cluster. The option is
     *                        provided since it can be of use in the future to produce dendograms and similar information.
     * @param previous a (possibly null) list of previous clustering obtained. If the list is non-null then the
     *                 representatives of the previous cluster would be added as zero weight points, ensuring that
     *                 the summarization is more smooth (in contrast to two independent summarizations). The zero weight
     *                 points of the past can serve as representatives of the current clustering.
    ...
    public List<ICluster<float[]>> summarize(int maxAllowed, double shrinkage, int numberOfRepresentatives, double separationRatio, List<ICluster<float[]>> previous){
        return stateCoordinator.getStore().summarize(maxAllowed,shrinkage,numberOfRepresentatives,separationRatio,previous);
    }
    ...
    public List<ICluster<float[]>> summarize(int maxAllowed, double shrinkage, int numberOfRepresentatives, List<ICluster<float[]>> previous){
        return stateCoordinator.getStore().summarize(maxAllowed,shrinkage,numberOfRepresentatives, DEFAULT_SEPARATION_RATIO_FOR_MERGE,previous);
    }
