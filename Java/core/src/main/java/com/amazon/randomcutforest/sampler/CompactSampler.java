...
/**
 * CompactSampler is an implementation of time-based reservoir sampling. When a point is submitted
 * to the sampler, the decision to accept the point gives more weight to newer points compared to
 * older points. The newness of a point is determined by its sequence index, and larger sequence
 * indexes are considered newer.
 *
 ...
 * </ol>
 *
 * <p>The coefficient function used by CompactSampler is: <code>
 * c(i) = exp(lambda * sequenceIndex(i))</code>.
 *
 * <p>Note that the sampling algorithm used is the same as for {@link SimpleStreamSampler}. The
 * difference is that CompactSampler is specialized to use Integers as the point reference type and
 * the implementation uses less runtime memory.
 */
public class CompactSampler extends AbstractStreamSampler<Integer> {

    /**
     * When creating a {@code CompactSampler}, the user has the option to disable storing sequence
     * indexes. If storing sequence indexes is disabled, then this value is used for the sequence
     * index in {@link ISampled} instances returned by {@link #getSample()}, {@link
     * #getWeightedSample()}, and {@link #getEvictedPoint()}.
     */
    public static final long SEQUENCE_INDEX_NA = -1L;

    /**
     * A max-heap containing the weighted points currently in sample. The head element is the lowest
     * priority point in the sample (or, equivalently, is the point with the greatest weight).
     */
    protected final float[] weight;

    /**
     * Index values identifying the points in the sample. See {@link
     * com.amazon.randomcutforest.store.IPointStore}.
     */
    ...
    /** If true, then the sampler will store sequence indexes along with the sampled points. */
     ...
     * this helps control the initial behavior of the points and ensure robustness by ensuring that
     * the samplers do not all sample the initial set of points.
     */
     ...
     * @param lambda The decay factor used for generating the weight of the point. For greater
     ...
     * @param storeSequenceIndexesEnabled If true, then the sequence indexes of sampled points will
     *     be stored in the sampler.
     */
    public CompactSampler(
            int sampleSize, double lambda, long seed, boolean storeSequenceIndexesEnabled) {
        this(
                sampleSize,
                lambda,
                new Random(seed),
     ...
     * @param storeSequenceIndexesEnabled If true, then the sequence indexes of sampled points will
     *     be stored in the sampler.
     * @param initialAcceptFraction the fraction of points admitted even when the sampler can accept
     *     a point
     */
    public CompactSampler(
            int sampleSize,
     ...
     * @param storeSequenceIndexesEnabled If true, then the sequence indexes of sampled points will
     *     be stored in the sampler.
     * @param initialAcceptFraction the fraction of points admitted to the sampler even when the
     *     sampler can accept
     */
    public CompactSampler(
            int sampleSize,
    ...
    public CompactSampler(
            int sampleSize, double lambda, Random random, boolean storeSequenceIndexesEnabled) {
        this(
                sampleSize,
                lambda,
                random,
    ...
    /**
     * This convenience constructor creates a SimpleStreamSampler with lambda equal to 0, which is
     * equivalent to uniform sampling on the stream.
     *
     * @param sampleSize The number of points in the sampler when full.
     * @param seed The seed value used to create a random number generator.
     * @param storeSequenceIndexesEnabled If true, then the sequence indexes of sampled points will
     ...
     */
    public static CompactSampler uniformSampler(
            int sampleSize, long seed, boolean storeSequenceIndexesEnabled) {
    ...
    public boolean acceptPoint(long sequenceIndex) {
        ...
        if ((size < capacity
                        && rng.nextDouble() < initialAcceptFraction + 1 - 1.0 * size / capacity)
    ...
    /**
     * evicts the maximum weight point from the sampler. can be used repeatedly to change the size
     * of the sampler and associated tree
     */
    ...
    /**
     * Check to see if the weight at current index is greater than or equal to the weight at each
     * corresponding child index. If validate is true then throw an IllegalStateException, otherwise
     * swap the nodes and perform the same check at the next level. Continue until you reach a level
     * where the parent node's weight is greater than or equal to both children's weights, or until
     * there are no more levels to descend.
     *
     * @param startIndex The index of node to start with.
     * @param validate If true, a violation of the heap property will throw an
     *     IllegalStateException. If false, then swap nodes that violate the heap property.
     */
            ...
            if (weight[maxIndex] > weight[current]) {
                if (validate) {
                    throw new IllegalStateException(
                            "the heap property is not satisfied at index " + current);
                }
    ...
    /**
     * Return the list of sampled points. If this sampler was created with the {@code
     * storeSequenceIndexesEnabled} flag set to false, then all sequence indexes in the list will be
     * set to {@link #SEQUENCE_INDEX_NA}.
     *
    ...
    private Stream<Weighted<Integer>> streamSample() {
        ...
        return IntStream.range(0, size)
                .mapToObj(
                        i -> {
                            long index =
                                    sequenceIndex != null ? sequenceIndex[i] : SEQUENCE_INDEX_NA;
                            return new Weighted<>(pointIndex[i], weight[i], index);
                        });
    }

    /** removes the adjustments to weight in accumulated lambda and resets the updates to lambda */
    private void reset_weights() {
        if (accumuluatedTimeDecay == 0) return;
    ...
    public CompactSampler(Builder<?> builder) {
        ...
        if (builder.weight != null
                || builder.pointIndex != null
                || builder.weight != null
                || builder.sequenceIndex != null
            ...
            checkArgument(
                    !builder.storeSequenceIndexesEnabled
                            || builder.sequenceIndex.length == builder.capacity,
                    " incorrect state");
