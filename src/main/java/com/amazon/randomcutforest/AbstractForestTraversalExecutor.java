...
public abstract class AbstractForestTraversalExecutor {
    ...
    /**
     * Internal update method which submits the given point and sequence index to {@link TreeUpdater#update(double[], long)}
     * for each TreeUpdater managed by this executor.
     *
     * @param pointCopy The point values that were are using the update the forest. The name of this parameter is a
     *                  reminder that we should update the forest with a copy of the point that was passed in, since
     *                  the original point may be modified later.
     * @param sequenceIndex The sequence index to assign to this point. This should be a unique value for every point
     *                      submitted.
     */
     ...
     *
     * @param point          The point that defines the traversal path.
     * @param visitorFactory A factory method which is invoked for each tree to construct a multi-visitor.
     * @param accumulator    A function that combines the results from individual trees into an aggregate result.
     * @param finisher       A function called on the aggregate result in order to produce the final result.
     * @param <R>            The visitor result type. This is the type that will be returned after traversing each individual
     *                       tree.
     ...
     * @return The aggregated and finalized result after sending a visitor through each tree in the forest.
     */
    public abstract <R, S> S traverseForestMulti(double[] point, Function<RandomCutTree, MultiVisitor<R>> visitorFactory,
                                                 BinaryOperator<R> accumulator, Function<R, S> finisher);
