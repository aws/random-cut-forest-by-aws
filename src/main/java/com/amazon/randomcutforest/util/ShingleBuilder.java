...
/**
 * A utility class for creating shingled points, which are also referred to as shingles. A shingle consists of multiple
 * points appended together. If individual points have n dimensions, and we include k points in a shingle, then the
 * shingle will have size n * m.
 *
 * There are two strategies for shingling: sliding and cyclic. In a sliding shingle, new points are appended to the
 * end of the shingle, and old points are removed from the front. For example, if we have a shingle size of 4 which
 * currently contains the points a, b, c, and d, then we can represent the shingle as abcd. The following schematic
 * shows how the shingle is updated as we add new points e and f.
 * <pre>
 *     abcd => bcde
 *     bcde => cdef
 * </pre>
 *
 * With cycling shingling, when a new point is added to a shingle it overwrites the oldest point in the shingle.
 * Using the same setup as above, a cyclic shingle would be updated as follows:
 * <pre>
 *     abcd => ebcd
 *     ebcd => efcd
 * </pre>
...
public class ShingleBuilder {
     ...
     * @param cyclic      If true, the shingle will use cyclic updates. If false, it will use sliding updates.
     */
    ...
    /**
     * Create a ShingleBuilder with the given dimensions and shingleSize. The resulting builder uses sliding updates.
     * @param dimensions  The number of dimensions in the input points.
     * @param shingleSize The number of points to store in a shingle.
     */
    ...
    /**
     * Return the index where the next input point will be stored in the internal shingle buffer. If the ShingleBuilder
     * uses cyclic updates, this value indicates the current point in the cycle.
     *
