...
public class SimpleInterpolationVisitor implements Visitor<InterpolationMeasure> {
	 ...
	 * @param sampleSize
	 *            The sub-sample size used by the RandomCutTree that is scoring the
	 *            point
	 * @param pointMass
	 *            indicates the mass/duplicity of the current point
	 * @param centerOfMass
	 *            indicates if the tree has centerOfMass
	 */
	...
	public void accept(Node node, int depthOfNode) {
		...
		if (pointEqualsLeaf) {
			...
			theShadowBox = theShadowBox == null
					? sibling.getBoundingBox()
	 ...
	 *
	 * @param node/leafNode
	 *            corresponds to the node in the tree influencing the current point
	 * @param centerOfMass
	 *            feature flag describing if the center of mass is enabled in tree
	 *            in general this can be used for arbitrary extensions of the node
