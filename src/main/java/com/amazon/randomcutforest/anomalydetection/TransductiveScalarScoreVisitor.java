...
public class TransductiveScalarScoreVisitor extends DynamicScoreVisitor {
	 ...
	 *            dimensions given a BoundingBox when the tree was built. This must
	 *            be the same as the probabilies of Transductive inference during
	 *            scoring. For extenstions where these are different, see
	 *            SimulatedTransductiveScalarScoreVisitor
	 *
	 *            Note that scores are not normalized because the function ranges
	 *            are unknown as is the case with DynamicScoreVisitor
	 */
	 ...
	 *
	 * @param boundingBox
	 *            The bounding box that we are computing the probability of
	 *            separation from.
