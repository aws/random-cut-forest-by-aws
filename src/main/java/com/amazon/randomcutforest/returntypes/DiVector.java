...
/**
 * A DiVector is used when we want to track a quantity in both the positive and negative directions for each
 * dimension in a manifold. For example, when using a {@link AnomalyAttributionVisitor}
 * to compute the attribution of the anomaly score to dimension of the input point, we want to know if the anomaly
 * score attributed to the ith coordinate of the input point is due to that coordinate being unusually high or
 * unusually low.
 */
public class DiVector {
    ...
    /**
     * Construct a new DiVector with the given number of spatial dimensions. In the result, {@link #high} and
     * {@link #low} will each contain this many variates.
     *
    ...
    /**
     * Add the values of {@link #high} and {@link #low} from the right vector to the left vector and return the left
     * vector. This method is used to accumulate DiVector results.
     *
     * @param left  The DiVector we are modifying. After calling this method, the low and high values in the DiVector
     *              will contain a sum of the previous values and the corresponding values from the right vector.
     * @param right A DiVector that we want to add to the left vector. This DiVector is not modified by the method.
    ...
    /**
     * Return a new DiVector where each value in high and low is equal to z times the corresponding value in this
     * DiVector.
     *
    ...
    /**
     * If the L1 norm of this DiVector is positive, scale the values in high and low so that the new L1 norm is equal
     * to the target value. If the current L1 norm is 0, do nothing.
     *
    ...
    /**
     * Apply the given function to each component of DiVector. That is, each entry of both the high and low arrays is
     * transformed using this function.
     *
     * @param function A function to apply to every entry of the high and low arrays in this DiVector.
     */
