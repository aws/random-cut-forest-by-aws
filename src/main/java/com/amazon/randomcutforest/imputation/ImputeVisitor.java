...
public class ImputeVisitor implements MultiVisitor<double[]> {
	 ...
	 *
	 * @param queryPoint
	 *            The point with missing values that we want to impute.
	 * @param numberOfMissingValues
	 *            The number of missing values in the query point.
	 * @param missingIndexes
	 *            The indexes of the missing values in the query point.
	 */
	...
	public void accept(final Node node, final int depthOfNode) {
		...
		double probabilityOfSeparation = getProbabilityOfSeparation(node.getBoundingBox(), queryPoint);
	...
	public void acceptLeaf(final Node leafNode, final int depthOfNode) {
		...
		double probabilityOfSeparation = getProbabilityOfSeparation(leafNode.getBoundingBox(), queryPoint);
		if (probabilityOfSeparation <= 0) {
			if (depthOfNode == 0) {
				rank = 0;
			} else {
