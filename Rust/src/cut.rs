...
impl Cut {
    ...
    pub fn random_cut_and_separation(bounding_box: &BoundingBox, factor: f64, point: &[f32]) -> (Cut, bool) {

        let min_values = bounding_box.get_min_values();
        let max_values = bounding_box.get_max_values();
        let mut range: f64 = min_values.iter().zip(max_values)
            .zip(point).map(|((x, y), z)|
        ...
        while dim < point.len() {
            let minv = if point[dim] < min_values[dim] { point[dim] } else { min_values[dim] };
            let maxv = if point[dim] > max_values[dim] { point[dim] } else { max_values[dim] };
