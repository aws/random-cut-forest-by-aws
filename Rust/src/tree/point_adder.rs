use crate::RCFFloat;
use super::{Tree, Node, Cut, BoundingBox};
...
/// A mechanism for adding points to trees.
/// 
/// Initialized with a mutable reference to a tree, this struct contains all of
/// the instructions necessary to add points to trees. This code is factored
/// out from `Tree` for improved readability.
pub struct PointAdder<'a, T> {
    tree: &'a mut Tree<T>,
}

impl<'a, T: RCFFloat> PointAdder<'a, T> {
    pub fn new(tree: &'a mut Tree<T>) -> Self {
        PointAdder { tree: tree }
    }

    /// Add a point to the tree.
    /// 
    /// Calls the recursive function, `add_point_by_node()`, using the root
    /// node.
    pub fn add_point(&mut self, point: Vec<T>) -> AddResult {
        match self.tree.root_node() {
            ...
            None => {
                ...
                let node_key = self.tree.node_store_mut().insert(node);
                self.tree.set_root_node(Some(node_key));
    ...
    fn add_point_by_node(&mut self, point: Vec<T>, node_key: usize) -> AddResult {
        ...
        if !self.point_inside_node(&point, node_key) {
            let cut = Cut::new_random_cut(&merged_box, self.tree.rng_mut()).unwrap();
    ...
    fn insert_new_leaf(
        ...
        if let Some(parent_key) = parent_key {
            ...
            self.tree.set_root_node(Some(merged_node_key));
        }
    ...
    fn insert_node(&mut self, node: Node<T>) -> usize {
        self.tree.node_store_mut().insert(node)
    }
