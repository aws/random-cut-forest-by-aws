...
use super::{AddResult, DeleteResult, PointAdder, PointDeleter};
...
impl<T: RCFFloat> Tree<T> {
    ...
    ///
    /// See [`PointAdder`] for more information.
    ///
    /// # Examples
    ///
    /// ```
    ...
    pub fn add_point(&mut self, point: Vec<T>) -> AddResult {
        let mut adder = PointAdder::new(self);
        adder.add_point(point)
    }
    ...
    pub fn delete_point(&mut self, point: &Vec<T>) -> DeleteResult {
        let mut deleter = PointDeleter::new(self);
        deleter.delete_point(point)
    }
